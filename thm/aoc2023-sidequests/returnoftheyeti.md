...
```
And voila, we have our WPA password and answer to the second question: `Christmas`

### Further PCAP Analysis
With the WPA password, we can further analyze the PCAP.  Go to Wireshark Preferences, then Protocols, then IEEE 802.11.  On that screen, click "Edit" next to "Decryption Keys".  Hit the "+" symbol, and add the following: `wpa-pwd:Christmas`
...
```
Seems we just found the answer to the third question, the attacker used `mimikatz` to extract a "juicy" file (encryption certificate).
